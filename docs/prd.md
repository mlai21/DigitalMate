...
- **沉淀路径**：Agent 生成 Skill 草稿 → 用户在后台确认后入库（P1 阶段一律人工确认，避免沉淀错误做法；对应产品红线"能力扩展需确认"）。草稿来源有四类（除对话内明确要求安装且扫描 safe 的 GitHub 导入外，均入库为待确认状态）：① 对话内 save_skill 工具（用户明确要求记住某套做法时由主模型直接沉淀）；② 轮后轻量复盘提议（6.2）；③ 每日深度反思提议（当天反复出现且无对应 Skill 的任务模式）；④ 后台手工创建 / GitHub 导入（含对话内 install_skill，见 P2-7）。
- **使用路径**：接到任务时先检索 Skill 库，命中则按 Skill 执行；每次命中记录使用日志（使用次数、关联会话），作为后续自动修订的依据。
