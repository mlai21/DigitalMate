...
> 修订记录
>
> - v0.8（2026-07-14）：新增**稳定阅读与聊天附件**（P0-10）——① 输入框按实际高度为消息区预留空间，不得遮挡消息；② 用户阅读历史时不强制滚到最新位置，以新消息按钮提示；③ 输入框「+」第一期支持 JPEG/PNG/WebP 与 PDF/TXT/MD/JSON/CSV，附件保存在自有私有存储并真正进入主模型；④ 附件上传不构成联网授权，携带附件的上下文禁止工具注入与联网；⑤ CSV 仅作为对话文本上下文，不解冻 P2 表格任务；⑥ 对话轮次在执行 Agent 前持久化受理标记，进程重启后同一来源不得再次执行 Agent、搜索或工具。同步补充模型图片能力风险与供应商载荷合同边界。
...
| P0-2 | 流式对话 | 回复流式输出；支持多轮上下文；单次用户输入只创建一个 Agent 响应事务，拟人分段属于同一事务，不得重复整段回复或混入无关后台消息；执行前须持久化受理标记，断线、并发或进程重启后同一来源 ID 不得再次执行 Agent、搜索或工具 |
