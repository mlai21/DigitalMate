...
| P0-4 | 联网搜索 | Agent 可调用搜索工具获取实时信息，答案融入人设语气输出；**触发受严格纪律约束（见 5.4）：默认不搜，仅在用户显式要求或命中"必须实时信息"白名单时才允许调用，宁可不搜也不乱搜** |
...
> 背景：实际使用中搜索工具被频繁误触发——闲聊、常识问答、观点讨论等完全不需要实时信息的场景也去联网搜索，既慢又破坏真人感。仅靠提示词"软约束"已被证明不够，必须软硬结合。

**原则：默认不搜。搜索是例外，不是常态；宁可少搜漏搜，不可乱搜。**

| 层 | 机制 | 说明 |
|---|---|---|
| 触发白名单（软） | 系统提示中定义**允许搜索的封闭场景清单** | 仅限：① 用户显式要求（"搜一下""查一下最新的"）；② 时效性硬需求（天气、新闻、股价、赛事、营业信息等明确依赖当下数据）；③ 用户询问的具体事实明显超出模型知识截止时间。清单外一律禁止，提示词以"禁止条款"而非"可用工具介绍"的口吻书写 |
| 独立硬门控（硬） | 搜索工具调用**不直接放行**，先经一次独立的低成本模型门控判定（输入：用户消息 + 拟搜索的 query），判定"该场景是否真的需要实时信息" | 门控通过才实际执行搜索；不通过则丢弃该 tool call，Agent 基于已有知识与记忆继续作答。门控判定与结果全部留痕后台（为什么放行/为什么拦截），可审计可调优。门控用低成本模型，延迟与费用可控 |
| 积极度档位（配） | 后台提供搜索积极度档位：**保守（默认）/ 标准 / 关闭** | 保守 = 白名单 + 硬门控双重生效；关闭 = 除用户显式要求外一律不搜。档位即时生效，无需重启 |
| 显式要求直通 | 用户明确说"搜一下"时**跳过门控直接放行** | 保证"想让它搜的时候一定搜"，与"不想让它搜的时候一定不搜"同等重要 |

验收口径：在闲聊、常识问答、观点讨论类消息上搜索触发率应接近 0；用户显式要求搜索时 100% 执行。门控阈值与白名单边界在实际使用中调优（见开放问题 10.12）。
...
12. **搜索门控的边界与成本（v0.6）**：白名单场景清单的完备性、门控模型的误拦率/误放率、门控引入的额外延迟是否可接受，需在实际使用中持续调优；若低成本模型门控误判率过高，备选方案是升级门控模型或改为"搜索前向用户确认"交互。Skill 意图识别自动匹配的置信阈值同样需实测调优（见 6.3）。
