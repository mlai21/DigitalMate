...
> 修订记录
>
> - v0.8（2026-07-14）：新增**稳定阅读与聊天附件**（P0-10）——① 输入框按实际高度为消息区预留空间，不得遮挡消息；② 用户阅读历史时不强制滚到最新位置，以新消息按钮提示；③ 输入框「+」第一期支持 JPEG/PNG/WebP 与 PDF/TXT/MD/JSON/CSV，附件保存在自有私有存储并真正进入主模型；④ 附件上传不构成联网授权，携带附件的上下文禁止工具注入与联网；⑤ CSV 仅作为对话文本上下文，不解冻 P2 表格任务。同步补充模型图片能力风险与供应商载荷合同边界。
