...
13. **图片模型与供应商载荷合同（P0-10）**：只有模型目录明确声明且供应商文档已确认支持图片输入时才可发送图片；默认模型或自定义模型能力未知时必须在绑定附件前提示用户切换，不得误称可用。供应商未明确确认 OpenAI 兼容接口支持 Data URI 的模型，也不得仅凭接口形状推断可用；需完成真实凭据验收后再开放对应能力。
