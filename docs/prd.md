...
DigitalMate 当前阶段的核心差异是：**一个默认数字分身、一份隔离记忆，出现在你所有的对话入口里，并保持真人感**。数据模型同时保留 `agent_id` 边界，为未来增加第二个数字分身与独立记忆域预留能力；本期不开放多分身创建、协作或共享记忆。
...
不照搬的部分：TUI 组件体系、本地文件系统为中心的设计（本产品以云端对话与记忆为中心，记忆存 PostgreSQL 而非 Markdown 文件）；QwenPaw Python Runner 与多 Agent/子 Agent 编排。本期只运行一个默认数字分身，但保留 Console 的 Agent 数据通路和数据库 `agent_id` 隔离边界，未来多分身功能需另行评审后启用。
