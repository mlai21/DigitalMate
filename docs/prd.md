...
- **渠道运行节点**：只处理 iMessage、SIP 等专用系统或媒体环境，通过加密连接使用中心渠道运行时；不运行模型、记忆、工具或自主调度。
