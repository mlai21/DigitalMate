...
| 管理后台 | QwenPaw Console 风格分组侧栏；会话日志可下钻到单会话完整消息 + 工具调用；模型页支持目录选择 + 自定义 ID |
...
| 记忆容量自整理 | profile/agent_self 层带容量上限，超限由轻量模型合并压缩（Hermes 模式），失败时降级淘汰旧低置信条目 |
| 轮后轻量复盘 | 每轮对话后异步用轻量模型复盘，产出行为修正建议与 Skill 草稿（Hermes background review 模式） |
