...
Caddy 作为唯一生产入口接管 80/443：`.env` 中必须配置 `DOMAIN`（域名 A 记录需指向服务器 IP，安全组放行 80/443）和对应的 HTTPS `PUBLIC_BASE_URL`，Caddy 会自动申请并续期 Let's Encrypt 证书并将 HTTP 跳转到 HTTPS。Caddy 会覆盖注入原始 request URI，供兼容 API 在 Next.js 路径规范化前识别编码绕过；直连 Next.js 不具备这项严格保证，不应作为生产入口。
