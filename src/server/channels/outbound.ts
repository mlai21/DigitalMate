...
export async function sendChannelMessage(
  ...
  if (message.channel === "telegram") {
    await sendTelegramMessage(env, message.externalConversationId, text, signal);
    return;
  }
  if (message.channel === "slack") {
    await sendSlackMessage(env, message.externalConversationId, text, signal);
    return;
  }
  if (message.channel === "feishu") {
    await sendFeishuMessage(env, message.externalConversationId, text, signal);
    return;
  }
  if (message.channel === "dingtalk") {
    await sendDingTalkSessionWebhook(message, text, signal);
  }
}
...
async function sendFeishuMessage(
  ...
  const tenantAccessToken = await getFeishuTenantAccessToken(env, signal);
  signal?.throwIfAborted();
...
async function getFeishuTenantAccessToken(env: AppEnv, signal?: AbortSignal): Promise<string> {
...
async function sendDingTalkSessionWebhook(
  message: NormalizedChannelMessage,
  text: string,
  signal?: AbortSignal,
): Promise<void> {
