...
const DEFINITIONS: Record<ChannelType, ManifestDefinition> = {
  ...
  mqtt: {
    ...
    capabilities: [],
    platformFields: [
      stringField("host", "主机", "", { required: true }),
      numberField("port", "端口", 1_883, 1, 65_535),
      selectField(
        "transport",
        "传输方式",
        ["tcp", "tls", "ws", "wss"],
      ...
      nullableStringField("tls_ca_certs", "TLS CA 证书内容"),
      nullableStringField("tls_certfile", "TLS 客户端证书内容"),
      secretField("tls_keyfile", "TLS 私钥内容", true),
    ],
  },
  matrix: {
    label: "Matrix",
